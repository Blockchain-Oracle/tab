...
function statusError(status: typeof agents.$inferSelect.status) {
...
export async function reserveSignRequest(
  ...
  return db.transaction(async (transaction) => {
    ...
    const blockedStatus = statusError(agent.status);
...
export async function completePreSigningChecks(
  ...
  return db.transaction(async (transaction) => {
    ...
    const currentStatusError = statusError(agent.status);
    let failure: GateCode | "FLOAT_EMPTY" | undefined = currentStatusError?.code;
    if (!failure) {
      const [reserved] = await transaction
        ...
        .where(
          and(
            eq(receipts.agentId, agent.id),
            eq(receipts.cycleId, receipt.cycleId),
