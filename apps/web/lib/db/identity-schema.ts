...
export const apiKeys = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(${table.type} = 'publishable' and (
          (${table.env} = 'test' and ${table.prefix} = 'pk_test_')
          or (${table.env} = 'live' and ${table.prefix} = 'pk_live_')
        )) or (${table.type} = 'secret' and (
          (${table.env} = 'test' and ${table.prefix} = 'sk_test_')
